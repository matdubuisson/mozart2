...
void TypeInfoOf<OptName>::gCollect(GC gc, RichNode from, UnstableNode& to) const {
  ...
  stable->make<OptName>(gc->vm, gc, from.access<OptName>());
}
...
stable->make<OptName>(sc->vm, sc, from.access<OptName>());
  } else {
    to.init(sc->vm, from);
  }
}
...
inline
Space *  TypedRichNode<OptName>::home() {
...
inline
bool  TypedRichNode<OptName>::lookupFeature(VM vm, RichNode feature, nullable<UnstableNode &> value) {
  return _self.access<OptName>().lookupFeature(vm, feature, value);
}

inline
bool  TypedRichNode<OptName>::lookupFeature(VM vm, nativeint feature, nullable<UnstableNode &> value) {
  return _self.access<OptName>().lookupFeature(vm, feature, value);
}
...
inline
UnstableNode  TypedRichNode<OptName>::label(VM vm) {
...
inline
UnstableNode  TypedRichNode<OptName>::arityList(VM vm) {
  return _self.access<OptName>().arityList(vm);
}

inline
UnstableNode  TypedRichNode<OptName>::clone(VM vm) {
  return _self.access<OptName>().clone(_self, vm);
}

inline
UnstableNode  TypedRichNode<OptName>::waitOr(VM vm) {
  return _self.access<OptName>().waitOr(vm);
}

inline
bool  TypedRichNode<OptName>::testRecord(VM vm, RichNode arity) {
  return _self.access<OptName>().testRecord(vm, arity);
}

inline
bool  TypedRichNode<OptName>::testTuple(VM vm, RichNode label, size_t width) {
  return _self.access<OptName>().testTuple(_self, vm, label, width);
}

inline
bool  TypedRichNode<OptName>::testLabel(VM vm, RichNode label) {
...
inline
GlobalNode *  TypedRichNode<OptName>::globalize(VM vm) {
