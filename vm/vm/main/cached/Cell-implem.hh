...
inline
UnstableNode  TypedRichNode<Cell>::exchange(VM vm, RichNode newValue) {
  return _self.access<Cell>().exchange(vm, newValue);
}

inline
UnstableNode  TypedRichNode<Cell>::access(VM vm) {
  return _self.access<Cell>().access(vm);
}

inline
void  TypedRichNode<Cell>::assign(VM vm, RichNode newValue) {
