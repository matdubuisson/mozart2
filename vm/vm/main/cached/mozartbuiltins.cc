...
class ModDebugger: public BuiltinModule {
public:
  ModDebugger(VM vm): BuiltinModule(vm, "Debugger") {
    instanceHello.setModuleName("Debugger");

    UnstableField fields[1];
    fields[0].feature = build(vm, "hello");
    fields[0].value = build(vm, instanceHello);
    UnstableNode label = build(vm, "export");
    UnstableNode module = buildRecordDynamic(vm, label, 1, fields);
    initModule(vm, std::move(module));
  }
private:
  mozart::builtins::ModDebugger::Hello instanceHello;
};
void registerBuiltinModDebugger(VM vm) {
  auto module = std::make_shared<ModDebugger>(vm);
  vm->registerBuiltinModule(module);
}
