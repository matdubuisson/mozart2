...
StaticArray<StableNode> TypedRichNode<CodeArea>::getElementsArray() {
  return _self.access<CodeArea>().getElementsArray();
}

StableNode& TypedRichNode<CodeArea>::getElements(size_t i) {
...
inline
void  TypedRichNode<CodeArea>::getCodeAreaInfo(VM vm, size_t & arity, ProgramCounter & start, size_t & Xcount, StaticArray<StableNode> & Ks) {
  _self.access<CodeArea>().getCodeAreaInfo(vm, arity, start, Xcount, Ks);
}

inline
void  TypedRichNode<CodeArea>::getCodeAreaDebugInfo(VM vm, atom_t & printName, UnstableNode & debugData) {
...
inline
UnstableNode  TypedRichNode<CodeArea>::serialize(VM vm, SE se) {
  return _self.access<CodeArea>().serialize(vm, se);
}

inline
GlobalNode *  TypedRichNode<CodeArea>::globalize(VM vm) {
  return _self.access<CodeArea>().globalize(_self, vm);
}

inline
void  TypedRichNode<CodeArea>::setUUID(VM vm, const UUID & uuid) {
