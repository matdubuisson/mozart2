...
inline
bool  TypedRichNode<BuiltinProcedure>::equals(VM vm, RichNode right) {
...
inline
void  TypedRichNode<BuiltinProcedure>::callBuiltin(VM vm, size_t argc, UnstableNode ** args) {
...
inline
void  TypedRichNode<BuiltinProcedure>::getCallInfo(VM vm, size_t & arity, ProgramCounter & start, size_t & Xcount, StaticArray<StableNode> & Gs, StaticArray<StableNode> & Ks) {
  _self.access<BuiltinProcedure>().getCallInfo(_self, vm, arity, start, Xcount, Gs, Ks);
}

inline
void  TypedRichNode<BuiltinProcedure>::getDebugInfo(VM vm, atom_t & printName, UnstableNode & debugData) {
  _self.access<BuiltinProcedure>().getDebugInfo(_self, vm, printName, debugData);
}

inline
UnstableNode  TypedRichNode<BuiltinProcedure>::serialize(VM vm, SE se) {
