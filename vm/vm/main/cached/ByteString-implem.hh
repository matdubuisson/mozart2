...
inline
bool  TypedRichNode<ByteString>::lookupFeature(VM vm, RichNode feature, nullable<UnstableNode &> value) {
  return _self.access<ByteString>().lookupFeature(vm, feature, value);
}

inline
bool  TypedRichNode<ByteString>::lookupFeature(VM vm, nativeint feature, nullable<UnstableNode &> value) {
  return _self.access<ByteString>().lookupFeature(vm, feature, value);
}
...
inline
bool  TypedRichNode<ByteString>::equals(VM vm, RichNode right) {
  return _self.access<ByteString>().equals(vm, right);
}

inline
int  TypedRichNode<ByteString>::compare(VM vm, RichNode right) {
...
inline
nativeint  TypedRichNode<ByteString>::stringCharAt(VM vm, RichNode offset) {
  return _self.access<ByteString>().stringCharAt(_self, vm, offset);
}

inline
UnstableNode  TypedRichNode<ByteString>::stringAppend(VM vm, RichNode right) {
  return _self.access<ByteString>().stringAppend(_self, vm, right);
}

inline
UnstableNode  TypedRichNode<ByteString>::stringSlice(VM vm, RichNode from, RichNode to) {
  return _self.access<ByteString>().stringSlice(_self, vm, from, to);
}

inline
void  TypedRichNode<ByteString>::stringSearch(VM vm, RichNode from, RichNode needle, UnstableNode & begin, UnstableNode & end) {
  _self.access<ByteString>().stringSearch(_self, vm, from, needle, begin, end);
}

inline
bool  TypedRichNode<ByteString>::stringHasPrefix(VM vm, RichNode prefix) {
  return _self.access<ByteString>().stringHasPrefix(vm, prefix);
}

inline
bool  TypedRichNode<ByteString>::stringHasSuffix(VM vm, RichNode suffix) {
