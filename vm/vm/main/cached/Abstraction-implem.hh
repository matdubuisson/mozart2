...
StaticArray<StableNode> TypedRichNode<Abstraction>::getElementsArray() {
  return _self.access<Abstraction>().getElementsArray();
}

StableNode& TypedRichNode<Abstraction>::getElements(size_t i) {
  return _self.access<Abstraction>().getElements(i);
}

inline
Space *  TypedRichNode<Abstraction>::home() {
...
inline
void  TypedRichNode<Abstraction>::getCallInfo(VM vm, size_t & arity, ProgramCounter & start, size_t & Xcount, StaticArray<StableNode> & Gs, StaticArray<StableNode> & Ks) {
  _self.access<Abstraction>().getCallInfo(vm, arity, start, Xcount, Gs, Ks);
}

inline
void  TypedRichNode<Abstraction>::getDebugInfo(VM vm, atom_t & printName, UnstableNode & debugData) {
...
inline
UnstableNode  TypedRichNode<Abstraction>::serialize(VM vm, SE se) {
  return _self.access<Abstraction>().serialize(vm, se);
}

inline
GlobalNode *  TypedRichNode<Abstraction>::globalize(VM vm) {
  return _self.access<Abstraction>().globalize(_self, vm);
}

inline
void  TypedRichNode<Abstraction>::setUUID(VM vm, const UUID & uuid) {
